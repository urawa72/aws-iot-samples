...
async function execute_register_thing(
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const map: { [key: string]: string } = JSON.parse(
        argv.template_parameters
      );
...
async function execute_csr(
  identity: iotidentity.IotIdentityClient,
  argv: Args
) {
  return new Promise(async (resolve, reject) => {
    try {
      function csrAccepted(
        error?: iotidentity.IotIdentityError,
        response?: iotidentity.model.CreateCertificateFromCsrResponse
      ) {
        if (response) {
          console.log(
            "CreateCertificateFromCsrResponse for certificateId=" +
              response.certificateId
          );
          fs.writeFileSync(
            `${response.certificateId}-certificate.pem.crt`,
      ...
      function csrRejected(
        error?: iotidentity.IotIdentityError,
        response?: iotidentity.model.ErrorResponse
      ) {
        if (response) {
          console.log(
            "CreateCertificateFromCsr ErrorResponse for " +
              "statusCode=:" +
              response.statusCode +
              " errorCode=:" +
              response.errorCode +
      ...
      console.log(
        "Subscribing to CreateCertificateFromCsr Accepted and Rejected topics.."
      );
      ...
      const csrRequest: iotidentity.model.CreateCertificateFromCsrRequest = {
...
async function main(argv: Args) {
  ...
  if (argv.use_websocket) {
    ...
    if (argv.proxy_host) {
      proxy_options = new http.HttpProxyOptions(
        argv.proxy_host,
        argv.proxy_port
      );
    }
